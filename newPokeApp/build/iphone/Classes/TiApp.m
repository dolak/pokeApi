...
- (void)boot
{
	DebugLog(@"[INFO] %@/%@ (%s.ec79bc5)",TI_APPLICATION_NAME,TI_APPLICATION_VERSION,TI_VERSION_STR);
...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *knewPokeAppUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",knewPokeAppUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a newPokeApp app using the node.js CLI
