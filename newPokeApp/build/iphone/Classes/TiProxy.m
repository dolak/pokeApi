...
//Rare exceptions to indicate a bug in the _newpokeapp code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_newpokeapp = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_newpokeapp]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.newpokeapp.module"
