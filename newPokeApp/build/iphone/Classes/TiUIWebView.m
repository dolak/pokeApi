...
static NSString * const knewPokeAppJavascript = @"Ti.App={};Ti.API={};Ti.App._listeners={};Ti.App._listener_id=1;Ti.App.id=Ti.appId;Ti.App._xhr=XMLHttpRequest;"
...
- (NSString *)_newpokeappInjection
{
	...
	[html appendString:knewPokeAppJavascript];
...
+ (NSString *)content:(NSString *)content withInjection:(NSString *)injection
{
	...
	// attempt to make well-formed HTML and inject in our newPokeApp bridge code
	   ...
	   baseURL:(NSURL*)baseURL
{
	...
	content = [[self class] content:content withInjection:[self _newpokeappInjection]];
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
	...
	if ([scheme hasPrefix:@"http"] || [scheme isEqualToString:@"ftp"]
		...
		if ([scheme isEqualToString:@"file"] || [scheme isEqualToString:@"app"]) {
			[NSURLProtocol setProperty:[self _newpokeappInjection] forKey:kContentInjection inRequest:(NSMutableURLRequest *)request];
		}
		return YES;
	}
